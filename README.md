...
В этом примере мы последовательно считываем строки из потока ввода и складываем их в общий список `strings` до тех пор, пока не встретим пустую строку. При этом не происходит никаких копирований объектов строк, так как класс поддерживает mode-семантику. После мы сортируем полученный список с помощью имплементированного итератора для динамических строк в обратном лексикографический порядке без учета регистра символов.
